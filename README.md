...
<details>
  <summary>📥 교직원 회원가입 및 승인 흐름</summary>

  교직원은 로컬 회원가입 또는 OAuth(카카오/구글) 로그인 후  
  추가 정보를 입력하고 총괄 관리자의 승인을 받아야 시스템 이용이 가능합니다.

  - Local 회원가입: 이름, 군번, 비밀번호 입력 → 가입 → 승인 대기  
  - OAuth 로그인: 로그인 → 추가 정보 입력 → 승인 대기  
  - 승인 전에는 로그인 제한 안내가 표시됩니다.

  ![교직원 회원가입 흐름](./images/admin-signup-approval.gif)
</details>
